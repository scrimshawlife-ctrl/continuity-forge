...
Usage:
  python retrieve_symbolic_patterns.py --brief brief.yaml
...
def compute_score(pattern: Dict, brief: Dict, weights: Dict) -> Dict:
    score_components = {}
    total = 0.0
...
def rank_patterns(brief: Dict, patterns_db: Dict) -> List[Dict]:
    ...
    for pid, p in patterns_db.items():
        # Basic matching from index hints
        score_data = compute_score(p, brief, DEFAULT_WEIGHTS)
        if score_data["total_score"] >= 0.3:  # loose prefilter
            candidates.append({
                "pattern": p,
